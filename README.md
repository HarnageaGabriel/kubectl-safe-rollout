...
| `scheduling-constraints-feasibility` | high/medium/low | The workload's `topologySpreadConstraints`, required pod self-anti-affinity, and `nodeSelector` / required node affinity against the live node topology (labels, cordon status, taints). Catches a spread, anti-affinity, or node-selection rule that can never be satisfied by the cluster's current nodes — high for a guaranteed deadlock (including a `nodeSelector`/required node affinity that matches no node at all), medium when only cordoned nodes stand between it and feasibility, low for a `ScheduleAnyway` constraint that is dead configuration. This is the first check that needs cluster-scoped `list` on Nodes, a materially wider ask than any other check here (every other check reads only within the namespace, or `get`s one named cluster-scoped object); it skips cleanly, like every other check, when that permission is denied. |
