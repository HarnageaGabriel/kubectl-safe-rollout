...
`<kind>` accepts `deployment`, `statefulset`, or `daemonset` (and their usual short/plural aliases, e.g. `deploy`, `sts`, `ds`). `check` and `watch` are e2e-verified for Deployment and StatefulSet (see "Verified and not verified" below).
...
| `hpa-quota-headroom` | medium | A HorizontalPodAutoscaler targeting this workload, its `maxReplicas`, and the same live ResourceQuota values as `quota-headroom`. An HPA can scale this workload up independently of any rollout; if that happens to coincide with a rollout window, the same quota wall blocks new pods, a trigger `quota-headroom`'s surge-only calculation cannot see. Medium, not high: an eroded margin, not a guaranteed blockage of the current rollout. Never evaluated for a DaemonSet target: DaemonSet has no `/scale` subresource, so an HPA targeting one can never actually function, and reporting headroom on that false premise would be wrong (same underlying limitation as `pdb-daemonset-scale`). |
| `pdb-daemonset-scale` | high/medium | DaemonSet only: PodDisruptionBudgets in the namespace whose selector matches the DaemonSet's pods, and (for a high finding) that PDB's own `status.conditions`. DaemonSet pods have no `/scale` subresource, so the in-tree disruption controller cannot compute a scale for them: a PDB expressed with `maxUnavailable` (any form) or a percentage `minAvailable` fails that lookup on every reconcile, permanently reporting `disruptionsAllowed: 0` with condition `DisruptionAllowed=False`/`SyncFailed`. An integer `minAvailable` never enters that code path and produces no finding. High when the `SyncFailed` condition is already observed on the live object; medium when not yet confirmed at the current generation. |
| `selector-overlap` | medium | Every other Deployment/StatefulSet in the namespace, whose pod selector or pod-template labels overlap this workload's (in either direction). Not "two controllers steal each other's live pods" — verified false in vanilla Kubernetes, ControllerRef blocks that — but an orphaned ReplicaSet/ControllerRevision (a `--cascade=orphan` delete, a Helm/Argo adoption mishap, GC lag) matching the overlapping selector gets silently adopted and scaled to zero (or folded into StatefulSet revision history), which Kubernetes' own controllers log as `user error!`. Reproduced live on kind: an orphaned ReplicaSet was adopted by the colliding Deployment and scaled 2→0 within seconds. Medium, not high: no live blockage today, since `spec.selector` is immutable and the overlap only bites on an orphaning event. |
...
| `priorityclass-exists` | high | The cluster-scoped PriorityClass named by the pod template's `priorityClassName`, if any (the two special keywords `system-node-critical`/`system-cluster-critical` are always valid and skipped). Not a heuristic: verified on kind, a workload naming one that does not exist will never create a single Pod, the same failure shape as `serviceaccount-exists`. |
...
| `ingressclass-exists` | high | The cluster-scoped IngressClass named by a relevant Ingress's `spec.ingressClassName`, if set (the deprecated `kubernetes.io/ingress.class` annotation is out of scope). Not a heuristic: verified on kind, a name that does not resolve means no ingress controller implements that Ingress at all, with no error surfaced anywhere. |
...
| `storageclass-exists` | high | StatefulSet only: the cluster-scoped StorageClass each `spec.volumeClaimTemplates` entry names, if set (a nil name means the cluster default, an explicit empty string means a pre-provisioned PersistentVolume — neither is flagged). Closes part of the gap `pvc-exists` documents explicitly (it only reads pod-template volumes, never `volumeClaimTemplates`): verifies the StorageClass reference is real, not that the per-ordinal PVCs themselves get created or bind, since those don't exist yet at check time. |
...
| `scheduling-constraints-feasibility` | high/medium/low | The workload's `topologySpreadConstraints`, required pod self-anti-affinity, and `nodeSelector` / required node affinity against the live node topology (labels, cordon status, taints). Catches a spread, anti-affinity, or node-selection rule that can never be satisfied by the cluster's current nodes — high for a guaranteed deadlock (including a `nodeSelector`/required node affinity that matches no node at all, or one whose only matching schedulable nodes all carry a `NoSchedule`/`NoExecute` taint the workload does not tolerate), medium when only cordoned nodes stand between it and feasibility, low for a `ScheduleAnyway` constraint that is dead configuration. This is the first check that needs cluster-scoped `list` on Nodes, a materially wider ask than any other check here (every other check reads only within the namespace, or `get`s one named cluster-scoped object); it skips cleanly, like every other check, when that permission is denied. |
| `admission-webhook-visibility` | high/medium/low | Cluster-scoped `ValidatingWebhookConfiguration`/`MutatingWebhookConfiguration` objects whose rules match this workload's pod `CREATE`, plus the `clientConfig.service` backend Service and its EndpointSlices when relevant. `failurePolicy` governs only errors calling a webhook, never an explicit deny from a healthy one, so this never promises "the rollout will be blocked" in general: high when a matching validating webhook has effective `failurePolicy: Fail` and its backend Service does not exist (a guaranteed call-error block, reproduced on kind); medium for the same case but with a Service that exists with zero ready endpoints; low, informational, for a matching mutating webhook (the pod actually created may differ from the template) or a matching validating webhook whose backend is healthy (in the path, but no blockage guaranteed). Webhooks declaring CEL `matchConditions`, or using an external `clientConfig.url`, are excluded from every conclusion. |
...
Verified: 20 Deployment end-to-end scenarios on kind v0.32.0 with containerd 2.3.1, all passing against v1.36.1, and the full suite (Deployment, StatefulSet, and `scheduling-constraints-feasibility` scenarios together) has since been run in full against **three Kubernetes minor versions** — v1.36.1, v1.35.5 and v1.34.8 — and passes cleanly on each, `serviceaccount-missing` included.
...
StatefulSet support (`test/e2e/statefulset_test.go`, 6 scenarios: crashloop, an ordinary multi-replica ordered rolling update completing with no finding, OnDelete-with-pending-update, partition-blocked-update, the `0 < partition < replicas` canary idiom completing with no finding, and quota-exhausted with the finding correctly attributed to the StatefulSet itself) has now been run against all three pinned Kubernetes minors above, alongside the full suite, with zero Deployment-path regression — first verified once on kind v0.33.0/Kubernetes v1.37.0/containerd 2.3.4, then across v1.36.1, v1.35.5 and v1.34.8. `check statefulset/<name>` was also smoke-tested manually against a live StatefulSet.
...
- StatefulSet's `RolloutComplete()` logic against `k8s.io/kubectl`'s actual vendored source (it replicates the publicly documented contract instead, since `k8s.io/kubectl` is not a dependency of this module — see the comment on `internal/workload/statefulset.go`'s `RolloutComplete`). It has behaved correctly across every scenario exercised on all three pinned minors, which reduces but does not eliminate the risk stated in that comment.
