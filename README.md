...
| `scheduling-constraints-feasibility` | high/medium/low | The workload's `topologySpreadConstraints` and required pod self-anti-affinity against the live node topology (labels, cordon status, taints). Catches a spread or anti-affinity rule that can never be satisfied by the cluster's current nodes — high for a guaranteed deadlock, medium when only cordoned nodes stand between it and feasibility, low for a `ScheduleAnyway` constraint that is dead configuration. This is the first check that needs cluster-scoped `list` on Nodes, a materially wider ask than any other check here (every other check reads only within the namespace, or `get`s one named cluster-scoped object); it skips cleanly, like every other check, when that permission is denied. |
