...
// live object. Deployment, StatefulSet and DaemonSet are supported: other
// kinds return an explicit error instead of silently providing partial
// behavior, so users know they must wait for future support instead of
...
func ResolveWorkload(ctx context.Context, clientset kubernetes.Interface, namespace, ref string) (workload.Workload, error) {
	...
	case "daemonset", "daemonsets", "ds", "ds.apps", "daemonset.apps":
		d, err := clientset.AppsV1().DaemonSets(namespace).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
			return nil, fmt.Errorf("reading DaemonSet %s/%s: %w", namespace, name, err)
		}
		return workload.FromDaemonSet(d), nil
	default:
		return nil, fmt.Errorf("kind %q is not supported in this version (only Deployment, StatefulSet and DaemonSet are supported)", kind)
	}
}
