...
// serviceAccountMissing matches the message generated by k8s.io/apiserver's
// ServiceAccount admission plugin when a ReplicaSet's pod template names a
// ServiceAccount that does not exist in the namespace (deleted, renamed, or
// never created after cloning a namespace). Captured verbatim from a kind
// cluster: "error looking up service account <ns>/<name>: serviceaccount
// "<name>" not found". Anchored on "error looking up service account"
// specifically, not the generic trailing "not found", which several other
// FailedCreate causes also end with.
var serviceAccountMissing = regexp.MustCompile(`error looking up service account [^/]+/([^:]+):`)

// ServiceAccountMissing reports the missing ServiceAccount's name, if the
// message matches. This is a separate, common, and fully deterministic
// cause of Reason=="FailedCreate": worth naming precisely rather than
// falling into the same "-undetermined" bucket as a genuinely unrecognized
// admission rejection.
func ServiceAccountMissing(message string) (name string, ok bool) {
	m := serviceAccountMissing.FindStringSubmatch(message)
	if m == nil {
		return "", false
	}
	return m[1], true
