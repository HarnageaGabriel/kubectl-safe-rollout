...
//
// StatefulSet and DaemonSet have no progressDeadlineSeconds field and no
// controller-set Progressing condition at all (see
// Workload.ProgressDeadlineExceeded's own doc comment): ok=false from that
// method is therefore ambiguous on its own — it also means "applicable to
// this kind, just not exceeded right now" for Deployment. Gating on
// target.Workload.Kind() directly, instead of treating every ok=false as a
// Skip, keeps that distinction intact: a Deployment mid-rollout that has not
// yet exceeded its deadline must still silently report no Finding, not a
// Skip.
...
func (ProgressDeadline) Diagnose(_ context.Context, target Target) (Result, error) {
	...
	case "StatefulSet", "DaemonSet":
		return SkipResult(ProgressDeadlineDiagnoserID, fmt.Sprintf("%s has no progressDeadlineSeconds and its controller sets no Progressing condition: this category cannot be evaluated for this kind", target.Workload.Kind())), nil
	}
