...
func (Paused) Diagnose(_ context.Context, target Target) (Result, error) {
	...
	case "StatefulSet", "DaemonSet":
		// Neither kind has a spec.paused field, full stop: unlike
		...
		return SkipResult(PausedDiagnoserID, fmt.Sprintf("%s has no spec.paused field: this category cannot be evaluated for this kind", target.Workload.Kind())), nil
	}
